...
(function (global, factory) {
  ...
  var _setupEvents = (function () {
    ...
    function _onTouchStart(e) {
      ...
      if (target.shouldPullToRefresh() == false) {
        return;
      }
    ...
    if (_shared.pointerEventsEnabled && _shared.supportsPointerEvents) {
      document.querySelectorAll(".pull-element").forEach(function (element) {
        element.addEventListener('pointerup', _onTouchEnd);
        element.addEventListener('pointerdown', _onTouchStart);
        element.addEventListener('pointermove', _onTouchMove, _passiveSettings);
      });
    } else {
      document.querySelectorAll(".pull-element").forEach(function (element) {
        element.addEventListener('touchend', _onTouchEnd);
        element.addEventListener('touchstart', _onTouchStart);
        element.addEventListener('touchmove', _onTouchMove, _passiveSettings);
      });
    }

    // Add a scroll event listener to the elements with class ".pull-parent"
    document.querySelectorAll(".pull-parent").forEach(function (element) {
    ...
    return {
      ...
      destroy: function destroy() {
        if (_shared.pointerEventsEnabled && _shared.supportsPointerEvents) {
          document.querySelectorAll(".pull-element").forEach(function (element) {
            element.removeEventListener('pointerdown', _onTouchStart);
            element.removeEventListener('pointerup', _onTouchEnd);
            element.removeEventListener('pointermove', _onTouchMove, _passiveSettings);
          });
        } else {
          document.querySelectorAll(".pull-element").forEach(function (element) {
            element.removeEventListener('touchstart', _onTouchStart);
            element.removeEventListener('touchend', _onTouchEnd);
            element.removeEventListener('touchmove', _onTouchMove, _passiveSettings);
          });
        }

        document.querySelectorAll(".pull-parent").forEach(function (element) {
          element.removeEventListener('scroll', _onScroll);
        });
      }
    };
  });
